...
async function decisionAgent(state) {
    ...
    if (!p || !f)
        throw new Error("Decision agent requires complete prior analysis");
    const llm = (0, llm_1.getLLM)({ temperature: 0.05 }); // very low temp for consistent decisions
    const structuredLLM = llm.withStructuredOutput(investment_1.InvestmentDecisionSchema);
