...
exports.analystMemoAgent = analystMemoAgent;
const llm_1 = require("../lib/llm");
const investment_1 = require("../types/investment");
async function analystMemoAgent(state) {
    const { companyProfile: p, financialAnalysis: f, newsAnalysis: n, riskAnalysis: r, investmentDecision: d } = state;
    if (!p || !f || !d)
        throw new Error("Analyst memo requires complete analysis");
    const llm = (0, llm_1.getLLM)({ temperature: 0.3 }); // slightly higher temp for professional writing quality
    const structuredLLM = llm.withStructuredOutput(investment_1.AnalystMemoSchema);
...
//# sourceMappingURL=analystMemoAgent.js.map