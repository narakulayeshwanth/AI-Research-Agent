...
async function bullBearAgent(state) {
    const { companyProfile: p, financialAnalysis: f, newsAnalysis: n, riskAnalysis: r } = state;
    if (!p)
        throw new Error("BullBear agent requires company profile");
    const llm = (0, llm_1.getLLM)({ temperature: 0.2 });
    const structuredLLM = llm.withStructuredOutput(investment_1.BullBearAnalysisSchema);
...
//# sourceMappingURL=bullBearAgent.js.map